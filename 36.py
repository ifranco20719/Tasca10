...
while latevaproposta != codi:
    ...
    encerts = 0
    coincidencies = 0
    coincidencia_pos = [False] * 4  # Tracks if a guessed digit is in the correct position
    # Verifiquem quants encerts hi ha hagut i quantes coincidències.
    for i in range(4):
        if latevaproposta[i] == codi[i]:
            encerts += 1
            coincidencia_pos[i] = True  # Mark the guessed digit in the correct position
        elif latevaproposta[i] in codi:
            coincidencies += 1
    # Display feedback with positions and coincidences
    print("La teva proposta (", latevaproposta, ") te", encerts, "encerts:", end=" ")
    for i in range(4):
        if coincidencia_pos[i]:
            print(" coincidència en posició", i+1, end=",")
    print(" i", coincidencies, "coincidències.")
